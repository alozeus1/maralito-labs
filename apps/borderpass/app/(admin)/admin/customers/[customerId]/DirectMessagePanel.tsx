...
import { sendStaffDirectMessage } from '../../../../actions/admin-messages';
import type { MessageView } from '../../../../actions/messages';

// Staff direct-message thread with a customer (general, not order-specific). Mirrors the order
// thread but posts sendStaffDirectMessage with the customer id. Optional photo (camera on mobile).
export function DirectMessagePanel({
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      const res = await sendStaffDirectMessage(fd);
      if (res.ok) {
        setBody('');
        pickFile(null);
        router.refresh();
  ...
  return (
    <section className="bg-surface-container-lowest shadow-level-1 p-md rounded-xl">
      <h2 className="font-heading text-headline-md mb-3">Direct message</h2>
