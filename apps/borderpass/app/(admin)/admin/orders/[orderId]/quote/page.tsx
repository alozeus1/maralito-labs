...
export default async function AdminOrderQuotePage({ params }: { params: Promise<{ orderId: string }> }) {
  ...
  return (
    <main className="p-6">
      ...
      <p className="mt-2 text-on-surface-variant">Create/edit draft, submit for approval, send (Phase 3 foundation).</p>

      <section className="mt-6 rounded-lg border border-outline p-4">
        <h2 className="font-medium">Payment (read-only)</h2>
        {!pay ? (
          <p className="mt-1 text-on-surface-variant">{res.ok ? 'No payment information.' : 'Payment status unavailable.'}</p>
        ) : (
          <dl className="mt-2 space-y-1 text-sm">
            <div className="flex justify-between"><dt className="text-on-surface-variant">Status</dt><dd className="font-medium">{paymentStatusLabel(pay.display_state)}</dd></div>
            {pay.amount_minor != null && pay.currency && (
              <div className="flex justify-between"><dt className="text-on-surface-variant">Amount</dt><dd>{formatMoney(pay.amount_minor, pay.currency)}</dd></div>
            )}
            {pay.payment_id && (
              <div className="flex justify-between"><dt className="text-on-surface-variant">Payment ref</dt><dd className="font-mono text-xs">{pay.payment_id}</dd></div>
            )}
            {pay.updated_at && (
              <div className="flex justify-between"><dt className="text-on-surface-variant">Updated</dt><dd>{new Date(pay.updated_at).toLocaleString()}</dd></div>
            )}
          </dl>
        )}
      </section>
