...
import { CreateInspection, InspectionIdParam, PassInspection, FailInspection } from '@maralito/schemas';
...
import { IllegalInspectionTransitionError, type InspectionStatus, type InspectionResult } from '@/domain/inspections/state-machine';
...
async function staffGuard() {
  ...
  try { requireAdminAccess(s); } catch { await writeAudit({ action: 'inspection.unauthorized_access_attempt', actorUserId: s?.sub }); return { s: null, err: { code: 'not_found', message: 'Not found.' } }; }
...
function asConflict<T>(fn: () => Promise<Result<T>>): Promise<Result<T>> {
  return fn().catch((e) => {
    if (e instanceof IllegalInspectionTransitionError) return { ok: false, error: { code: 'conflict_state', message: 'Illegal inspection transition.' } } as Result<T>;
    throw e;
  });
}
...
export async function createInspection(input: unknown): Promise<Result<{ inspection_id: string }>> {
  ...
  const parsed = CreateInspection.safeParse(input); if (!parsed.success) return { ok: false, error: { code: 'validation_failed', message: 'Invalid request.' } };
  return withTenant({ authUserId: s.sub, orgId: s.orgId }, async (tx) => {
    ...
    if (!canStartInspection(order.status as OrderStatus)) return { ok: false, error: { code: 'conflict_state', message: 'Order is not ready for inspection (must be paid).' } };
    const id = newId('insp');
    await tx.insert(inspections).values({ id, orgId: s.orgId, customerId: order.customerId, orderId: order.id, status: 'scheduled' });
    await writeAudit({ action: 'inspection.created', orgId: s.orgId, actorUserId: s.sub, actorRole: s.roles[0]!, entityType: 'inspection', entityId: id });
    await emitInspectionEvent('inspection.created', { inspection_id: id, order_id: order.id, status: 'scheduled' });
    return { ok: true, data: { inspection_id: id } };
  });
}

/** Shared: load an org inspection (staff RLS) and run a transition via the seam. */
async function runTransition(inspectionId: string, to: InspectionStatus, meta?: { reason?: string; customerSummary?: string }): Promise<Result> {
  const { s, err } = await staffGuard(); if (!s) return { ok: false, error: err! };
...
export async function startInspection(input: unknown): Promise<Result> {
  const p = InspectionIdParam.safeParse(input); if (!p.success) return { ok: false, error: { code: 'validation_failed', message: 'Invalid.' } };
...
export async function passInspection(input: unknown): Promise<Result> {
  const p = PassInspection.safeParse(input); if (!p.success) return { ok: false, error: { code: 'validation_failed', message: 'Invalid.' } };
...
export async function failInspection(input: unknown): Promise<Result> {
  const p = FailInspection.safeParse(input); if (!p.success) return { ok: false, error: { code: 'validation_failed', message: 'Invalid.' } };
  const meta = {
...
export async function getOrderInspectionForStaff(orderId: string): Promise<Result<StaffInspectionView | null>> {
  const { s, err } = await staffGuard(); if (!s) return { ok: false, error: err! };
  return withTenant({ authUserId: s.sub, orgId: s.orgId }, async (tx) => {
    const i = (await tx.select().from(inspections).where(eq(inspections.orderId, orderId)).orderBy(desc(inspections.createdAt)).limit(1))[0] ?? null;
    if (!i) return { ok: true, data: null };
    const view: StaffInspectionView = {
      inspection_id: i.id, status: i.status as InspectionStatus, result: (i.result as InspectionResult | null) ?? null,
      staff_notes: i.staffNotes ?? null, customer_summary: i.customerSummary ?? null,
