...
async function staffGuard() {
  ...
  try { requireAdminAccess(s); } catch { return { s: null, err: { code: 'not_found', message: 'Not found.' } }; }
...
export async function adminListOrders(): Promise<Result<{ id: string; order_ref: string; status: string; service_type: string }[]>> {
...
export async function adminGetOrder(orderId: string): Promise<Result<unknown>> {
  ...
  return withTenant({ authUserId: s.sub, orgId: s.orgId }, async (tx) => {
    ...
    return o ? { ok: true, data: o } : { ok: false, error: { code: 'not_found', message: 'Order not found.' } };
  });
}
...
export async function advanceOrder(input: unknown): Promise<Result> {
  const { s, err } = await staffGuard(); if (!s) return { ok: false, error: err! };
  const parsed = z.object({ order_id: z.string().regex(/^ord_/), to_status: z.enum(ORDER_STATUSES) }).safeParse(input);
  if (!parsed.success) return { ok: false, error: { code: 'validation_failed', message: 'Invalid request.' } };
  try { requireRole(s, 'operations_manager'); } catch { return { ok: false, error: { code: 'forbidden', message: 'Requires operations_manager.' } }; }
  return withTenant({ authUserId: s.sub, orgId: s.orgId }, async (tx) => {
    ...
    try {
      await transitionOrder(tx, { id: o.id, orgId: o.orgId, status: o.status as OrderStatus }, parsed.data.to_status, { userId: s.sub, role: 'operations_manager' });
    } catch (e) {
...
export async function holdOrder(orderId: string, reason: string): Promise<Result> {
  const { s, err } = await staffGuard(); if (!s) return { ok: false, error: err! };
  try { requireRole(s, 'operations_manager'); } catch { return { ok: false, error: { code: 'forbidden', message: 'Requires operations_manager.' } }; }
  await writeAudit({ action: 'order.hold', orgId: s.orgId, actorUserId: s.sub, actorRole: 'operations_manager', entityType: 'order', entityId: orderId, metadata: { reason } });
  return { ok: true };
}
