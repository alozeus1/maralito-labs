...
async function staffGuard() {
  ...
  try { requireAdminAccess(s); } catch { await writeAudit({ action: 'payment.unauthorized_access_attempt', actorUserId: s?.sub }); return { s: null, err: { code: 'not_found', message: 'Not found.' } }; }
 ...
 */
export async function getOrderPaymentForStaff(orderId: string): Promise<Result<StaffPaymentSummary>> {
