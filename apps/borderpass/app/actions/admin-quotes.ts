...
async function staffGuard() {
  ...
  try {
    ...
    await writeAudit({ action: 'quote.unauthorized_access_attempt', actorUserId: s?.sub });
...
const isFinance = (s: AppSession) =>
  s.roles.some((r) => (FINANCE_ROLE_KEYS as readonly string[]).includes(r));
/** The actual finance role the user holds (the role that satisfied the guard) — never positional roles[0]. */
const financeRoleOf = (s: AppSession) =>
  s.roles.find((r) => (FINANCE_ROLE_KEYS as readonly string[]).includes(r))!;
...
function asConflict<T>(fn: () => Promise<Result<T>>): Promise<Result<T>> {
  return fn().catch((e) => {
    if (e instanceof IllegalQuoteTransitionError)
      return {
        ok: false,
        error: { code: 'conflict_state', message: 'Illegal quote transition.' },
      } as Result<T>;
    throw e;
  });
}
...
async function recalc(
  tx: Parameters<Parameters<import('@maralito/db').Db['transaction']>[0]>[0],
  quoteId: string,
) {
  const lines = await tx.select().from(quoteLineItems).where(eq(quoteLineItems.quoteId, quoteId));
  const ql: QuoteLine[] = lines.map((l) => ({
    kind: l.kind,
    total_amount_minor: l.totalAmountMinor,
    currency: l.currency,
    customer_visible: l.customerVisible,
...
export async function createQuoteDraft(input: unknown): Promise<Result<{ quote_id: string }>> {
  ...
  const parsed = CreateQuoteDraft.safeParse(input);
  if (!parsed.success)
    return {
      ok: false,
      error: {
  ...
  return withTenant({ authUserId: s.sub, orgId: s.orgId }, async (tx) => {
    ...
    await tx.insert(quotes).values({
      ...
      customerMessage: parsed.data.customer_message ?? null,
      internalNotes: parsed.data.internal_notes ?? null,
    });
    for (const it of parsed.data.items ?? []) {
      await tx.insert(quoteLineItems).values({
        ...
        unitAmountMinor: it.unit_amount_minor,
        totalAmountMinor: it.total_amount_minor,
        currency: it.currency,
        taxable: it.taxable ?? false,
        customerVisible: it.customer_visible ?? true,
        internalOnly: it.internal_only ?? false,
...
export async function updateDraftQuote(input: unknown): Promise<Result> {
  const { s, err } = await staffGuard();
  if (!s) return { ok: false, error: err! };
  const parsed = UpdateQuoteDraft.safeParse(input);
  if (!parsed.success)
  ...
  return withTenant({ authUserId: s.sub, orgId: s.orgId }, async (tx) => {
    ...
    if (!editable(q.status))
      return { ok: false, error: { code: 'conflict_state', message: 'Quote not editable.' } };
    await tx
      .update(quotes)
      .set({
        customerMessage: parsed.data.customer_message ?? null,
        internalNotes: parsed.data.internal_notes ?? null,
        requiresApproval: parsed.data.requires_approval ?? q.requiresApproval,
        updatedAt: new Date(),
      })
...
export async function addQuoteLineItem(input: unknown): Promise<Result> {
  ...
  if (!s) return { ok: false, error: err! };
  const parsed = z
    .object({ quote_id: z.string().regex(/^qte_/), item: QuoteLineItemInput })
    .safeParse(input);
  if (!parsed.success)
    return { ok: false, error: { code: 'validation_failed', message: 'Invalid line item.' } };
  return withTenant({ authUserId: s.sub, orgId: s.orgId }, async (tx) => {
    const q = await tx.query.quotes.findFirst({ where: eq(quotes.id, parsed.data.quote_id) });
    if (!q) return { ok: false, error: { code: 'not_found', message: 'Quote not found.' } };
    if (!editable(q.status))
    ...
    if (it.currency !== q.currency)
      return {
        ok: false,
        error: { code: 'validation_failed', message: 'Line currency must match quote.' },
      };
    await tx.insert(quoteLineItems).values({
      id: newId('qli'),
      quoteId: q.id,
...
export async function removeQuoteLineItem(input: unknown): Promise<Result> {
  ...
  if (!s) return { ok: false, error: err! };
  const parsed = z
    .object({ quote_id: z.string().regex(/^qte_/), line_id: z.string().regex(/^qli_/) })
    .safeParse(input);
  if (!parsed.success)
    return { ok: false, error: { code: 'validation_failed', message: 'Invalid.' } };
  return withTenant({ authUserId: s.sub, orgId: s.orgId }, async (tx) => {
    const q = await tx.query.quotes.findFirst({ where: eq(quotes.id, parsed.data.quote_id) });
    if (!q || !editable(q.status))
      return { ok: false, error: { code: 'conflict_state', message: 'Quote not editable.' } };
    await tx
      .delete(quoteLineItems)
      .where(and(eq(quoteLineItems.id, parsed.data.line_id), eq(quoteLineItems.quoteId, q.id)));
    await recalc(tx, q.id);
    await writeAudit({
      action: 'quote.line_item_removed',
      orgId: s.orgId,
      actorUserId: s.sub,
      actorRole: staffRoleOf(s),
      entityType: 'quote',
...
export async function submitQuoteForApproval(input: unknown): Promise<Result> {
  const { s, err } = await staffGuard();
  if (!s) return { ok: false, error: err! };
  const parsed = SubmitForApproval.safeParse(input);
  if (!parsed.success)
    return { ok: false, error: { code: 'validation_failed', message: 'Invalid.' } };
  return asConflict(() =>
    withTenant({ authUserId: s.sub, orgId: s.orgId }, async (tx) => {
      ...
      if (!q || q.status !== 'draft')
        return {
          ok: false,
          error: { code: 'conflict_state', message: 'Only drafts can be submitted.' },
        };
      ...
      const needs = requiresFinanceApproval({
        total_minor: q.totalMinor,
        discount_minor: q.discountMinor,
        has_manual_adjustment: lines.some((l) => l.kind === 'adjustment'),
        service_type: order?.serviceType ?? '',
        marked_requires_approval: q.requiresApproval,
      });
      ...
      await transitionQuote(
        { id: q.id, orderId: q.orderId, orgId: q.orgId, status: 'draft' },
        target,
        { userId: s.sub, role },
      );
      return { ok: true };
...
export async function approveQuote(input: unknown): Promise<Result> {
  ...
  if (!isFinance(s))
    return {
      ok: false,
      error: { code: 'forbidden', message: 'Requires finance/ops/super_admin.' },
    };
  const parsed = ApproveQuote.safeParse(input);
  if (!parsed.success)
    return { ok: false, error: { code: 'validation_failed', message: 'Invalid.' } };
  return withTenant({ authUserId: s.sub, orgId: s.orgId }, async (tx) => {
    const q = await tx.query.quotes.findFirst({ where: eq(quotes.id, parsed.data.quote_id) });
    if (!q || q.status !== 'pending_finance_approval')
      return { ok: false, error: { code: 'conflict_state', message: 'Not pending approval.' } };
    ...
    await transitionQuote(
      { id: q.id, orderId: q.orderId, orgId: q.orgId, status: 'pending_finance_approval' },
      'approved',
      { userId: s.sub, role: financeRoleOf(s) },
    );
    return { ok: true };
  });
}
...
export async function rejectQuote(input: unknown): Promise<Result> {
  ...
  const parsed = RejectQuote.safeParse(input);
  if (!parsed.success)
    return { ok: false, error: { code: 'validation_failed', message: 'Reason required.' } };
  return withTenant({ authUserId: s.sub, orgId: s.orgId }, async (tx) => {
    const q = await tx.query.quotes.findFirst({ where: eq(quotes.id, parsed.data.quote_id) });
    if (!q || q.status !== 'pending_finance_approval')
      return { ok: false, error: { code: 'conflict_state', message: 'Not pending approval.' } };
...
export async function sendQuote(input: unknown): Promise<Result> {
  const { s, err } = await staffGuard();
  if (!s) return { ok: false, error: err! };
  const parsed = SendQuote.safeParse(input);
  if (!parsed.success)
    return { ok: false, error: { code: 'validation_failed', message: 'Invalid.' } };
  return withTenant({ authUserId: s.sub, orgId: s.orgId }, async (tx) => {
    const q = await tx.query.quotes.findFirst({ where: eq(quotes.id, parsed.data.quote_id) });
    if (!q || q.status !== 'approved')
      return {
        ok: false,
        error: { code: 'conflict_state', message: 'Quote must be approved to send.' },
      };
    const order = await tx.query.orders.findFirst({ where: eq(orders.id, q.orderId) });
    if (parsed.data.expires_in_hours) {
      const exp = new Date(Date.now() + parsed.data.expires_in_hours * 3600_000);
      await withPrivilegedDbAccess('quote.set_expiry', async (db) => {
        await db.update(quotes).set({ expiresAt: exp }).where(eq(quotes.id, q.id));
      });
    }
    await transitionQuote(
      { id: q.id, orderId: q.orderId, orgId: q.orgId, status: 'approved' },
      'sent_to_customer',
    ...
    if (order && order.status === 'under_review') {
      await transitionOrderPrivileged(
        { id: order.id, orgId: order.orgId, status: order.status as OrderStatus },
        'quote_ready',
        { userId: s.sub, role: 'system' },
      );
    }
    return { ok: true };
  });
...
export async function cancelQuote(input: unknown): Promise<Result> {
  const { s, err } = await staffGuard();
  if (!s) return { ok: false, error: err! };
  const parsed = QuoteIdParam.safeParse(input);
  if (!parsed.success)
    return { ok: false, error: { code: 'validation_failed', message: 'Invalid.' } };
  return withTenant({ authUserId: s.sub, orgId: s.orgId }, async (tx) => {
    const q = await tx.query.quotes.findFirst({ where: eq(quotes.id, parsed.data.quote_id) });
    if (!q) return { ok: false, error: { code: 'not_found', message: 'Quote not found.' } };
    if (!canTransitionQuoteStatus(q.status as QuoteStatus, 'cancelled', s.roles[0]!))
      return { ok: false, error: { code: 'conflict_state', message: 'Cannot cancel.' } };
    await transitionQuote(
...
export async function supersedeQuote(input: unknown): Promise<Result> {
  ...
  return withTenant({ authUserId: s.sub, orgId: s.orgId }, async (tx) => {
    const q = await tx.query.quotes.findFirst({ where: eq(quotes.id, parsed.data.quote_id) });
    if (!q) return { ok: false, error: { code: 'not_found', message: 'Quote not found.' } };
    if (!canTransitionQuoteStatus(q.status as QuoteStatus, 'superseded', s.roles[0]!))
      return { ok: false, error: { code: 'conflict_state', message: 'Cannot supersede.' } };
    await transitionQuote(
      { id: q.id, orderId: q.orderId, orgId: q.orgId, status: q.status as QuoteStatus },
      'superseded',
      { userId: s.sub, role: s.roles[0]! },
