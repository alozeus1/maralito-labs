...
export async function getMyOrderDelivery(orderId: string): Promise<Result<DeliverySummaryView | null>> {
  const { s, err } = await custGuard(); if (!s) return { ok: false, error: err! };
  return withTenant({ authUserId: s.sub, orgId: s.orgId }, async (tx) => {
    const d = (await tx.select().from(deliveryPreparations).where(eq(deliveryPreparations.orderId, orderId)).orderBy(desc(deliveryPreparations.createdAt)).limit(1))[0] ?? null; // RLS → own
