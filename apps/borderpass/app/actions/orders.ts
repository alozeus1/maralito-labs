...
export async function createOrder(
  input: unknown,
): Promise<Result<{ order_id: string; order_ref: string }>> {
  ...
  return withTenant({ authUserId: s.sub, orgId: s.orgId }, async (tx) => {
    ...
    await tx.insert(orders).values({
      ...
      declaredValue: parsed.data.declared_value ?? null,
      deliveryAddressId: parsed.data.delivery_address_id ?? null,
      hubAddressId: parsed.data.hub_address_id ?? null,
      correlationId: id,
    });
...
export async function updateDraftOrder(input: unknown): Promise<Result> {
  ...
  if (!parsed.success)
    return { ok: false, error: { code: 'validation_failed', message: 'Invalid patch.' } };
  return withTenant({ authUserId: s.sub, orgId: s.orgId }, async (tx) => {
    ...
    if (o.status !== 'draft' && o.status !== 'missing_information')
      return { ok: false, error: { code: 'conflict_state', message: 'Order not editable.' } };
    ...
    await tx
      ...
      .set({
        ...
        ...(p.delivery_address_id !== undefined
          ? { deliveryAddressId: p.delivery_address_id }
          : {}),
...
export async function submitOrder(input: unknown): Promise<Result> {
  ...
  return withTenant({ authUserId: s.sub, orgId: s.orgId }, async (tx) => {
    ...
    const missing = submitMissingFields({
      ...
      hubAddressId: o.hubAddressId,
    });
    if (missing.length)
      return {
        ok: false,
        error: { code: 'validation_failed', message: 'Missing information.', details: missing },
      };
