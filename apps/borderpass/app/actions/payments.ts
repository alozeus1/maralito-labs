...
import {
  ...
  type SafePaymentIntentView,
} from '@maralito/payments';
 ...
 */
export async function getMyOrderPaymentSummary(
  orderId: string,
): Promise<Result<PaymentSummaryView>> {
  ...
  return withTenant({ authUserId: s.sub, orgId: s.orgId }, async (tx) => {
    ...
    const summary: PaymentSummaryView = {
      ...
      quote: {
        ...
        status: getCustomerVisibleQuoteStatus(q.status as QuoteStatus),
      },
...
export async function initiateQuotePayment(input: unknown): Promise<Result<SafePaymentIntentView>> {
  ...
  const loaded = await withTenant({ authUserId: s.sub, orgId: s.orgId }, async (tx) => {
    ...
    const chk = canInitiatePayment({
      quoteStatus: q.status as QuoteStatus,
  ...
  if ('code' in loaded) {
    ...
    return {
      ok: false,
      error: {
        code: loaded.code === 'not_found' ? 'not_found' : 'conflict_state',
        message: map[loaded.code] ?? 'Cannot initiate payment.',
      },
    };
  }
  const { q } = loaded;
  ...
  const existing = await withPrivilegedDbAccess(
    'payment.lookup',
    async (db) =>
      (
        await db
          .select()
          .from(payments)
          .where(and(eq(payments.provider, 'stripe'), eq(payments.idempotencyKey, idem)))
          .limit(1)
      )[0] ?? null,
  );
  ...
  if (existing?.stripePaymentIntentId) {
    ...
    amountOut = existing.amountMinor;
    currencyOut = existing.currency;
    ...
    if (existing) {
      ...
      const res = await withPrivilegedDbAccess('payment.initiate', async (db) => {
        const inserted = await db
          ...
          .onConflictDoNothing({ target: [payments.provider, payments.idempotencyKey] })
      ...
      if (res.kind === 'created') {
        ...
        paymentId = res.winner.id;
        amountOut = res.winner.amountMinor;
        currencyOut = res.winner.currency;
        statusOut = res.winner.status;
        ...
        return {
          ok: false,
          error: { code: 'conflict_state', message: 'Could not initiate payment, please retry.' },
        };
      }
    }
  }
  ...
  if (created) {
    await writeAudit({
      ...
      entityType: 'payment',
      entityId: paymentId,
      after: { amount_minor: amountOut, currency: currencyOut },
    });
    await emitPaymentEvent('payment.intent_created', {
      payment_id: paymentId,
      order_id: q.orderId,
      quote_id: q.id,
    });
