...
function customerQuoteView(q: typeof quotes.$inferSelect) {
  return {
    ...
    service_fee_minor: q.serviceFeeMinor,
    delivery_fee_minor: q.deliveryFeeMinor,
    estimated_tax_minor: q.estimatedTaxMinor,
    inspection_fee_minor: q.inspectionFeeMinor,
    discount_minor: q.discountMinor,
    expires_at: q.expiresAt,
    customer_message: q.customerMessage,
  };
}
...
export async function acceptQuote(input: unknown): Promise<Result> {
  ...
  if (!canCustomerAcceptQuote(q.status as QuoteStatus, q.expiresAt)) {
    await writeAudit({
      action: 'quote.invalid_transition_attempt',
      orgId: q.orgId,
      actorUserId: s.sub,
      actorRole: 'customer',
      entityType: 'quote',
      entityId: q.id,
      after: { attempted: 'accept', status: q.status },
    });
    return {
      ok: false,
      error: { code: 'conflict_state', message: 'Quote cannot be accepted (not sent or expired).' },
    };
  }
...
export async function declineQuote(input: unknown): Promise<Result> {
  const { s, err } = await custGuard();
  if (!s) return { ok: false, error: err! };
  const parsed = DeclineQuote.safeParse(input);
  if (!parsed.success)
    return { ok: false, error: { code: 'validation_failed', message: 'Invalid request.' } };
  ...
  if (!canCustomerDeclineQuote(q.status as QuoteStatus)) {
    await writeAudit({
      ...
      entityType: 'quote',
      entityId: q.id,
      after: { attempted: 'decline', status: q.status },
    });
