...
import {
  DELIVERY_PREP_STATUSES,
  isLegalDeliveryPrepTransition,
  assertDeliveryPrepTransition,
  IllegalDeliveryPrepTransitionError,
  isTerminalDeliveryPrepStatus,
  getNextAllowedDeliveryPrepStatuses,
...
describe('delivery-prep state machine', () => {
  ...
  it('assert throws on illegal', () =>
    expect(() => assertDeliveryPrepTransition('pending', 'handed_off')).toThrow(
      IllegalDeliveryPrepTransitionError,
    ));
