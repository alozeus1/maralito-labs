...
import { assertTransition, isLegalTransition, isTerminal, IllegalTransitionError, ORDER_STATUSES } from './state-machine';
...
describe('order state machine', () => {
  ...
  it('illegal transition throws', () => expect(() => assertTransition('draft', 'delivered')).toThrow(IllegalTransitionError));
...
describe('submit rules', () => {
  it('flags missing fields on empty draft', () => {
    ...
    expect(m).toEqual(expect.arrayContaining(['items', 'purpose', 'declared_value', 'delivery_address']));
  });
  it('passes a complete buy-for-me draft', () =>
    expect(submitMissingFields({ status: 'draft', serviceType: 'buy_for_me', itemCount: 1, purpose: 'personal', declaredValue: { amount_minor: 1500, currency: 'USD' }, deliveryAddressId: 'addr_1' })).toHaveLength(0));
