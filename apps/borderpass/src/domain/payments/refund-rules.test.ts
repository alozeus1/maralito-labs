  ...
  it('rejects over-refund, zero/negative, and non-refundable payment', () => {
    expect(
      canInitiateRefund({
        paymentStatus: 'succeeded',
        paymentAmountMinor: 10000,
        alreadyRefundedMinor: 7000,
        requestedMinor: 4000,
      }),
    ).toEqual({ ok: false, code: 'exceeds_remaining' });
    expect(
      canInitiateRefund({
        paymentStatus: 'succeeded',
        paymentAmountMinor: 10000,
    ...
    ).toEqual({ ok: false, code: 'payment_not_refundable' });
  });
