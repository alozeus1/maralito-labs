...
export interface AuditInput {
  action: string; orgId?: string; actorUserId?: string; actorRole?: string;
  ...
  try {
    ...
    await withPrivilegedDbAccess(`audit:${input.action}`, async (db) => {
      await db.insert(auditLogs).values({
        id: newId('aud'), action: input.action,
        orgId: input.orgId ?? null, actorUserId: input.actorUserId ?? null, actorRole: input.actorRole ?? null,
