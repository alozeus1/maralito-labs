...
import { assertPaymentTransition, paymentSucceeds, type PaymentStatus } from '@/domain/payments/state-machine';
...
export async function transitionPayment(
  ...
  meta?: { eventType?: string; providerEventId?: string; payloadSummary?: Record<string, unknown>; reason?: string },
): Promise<void> {
  assertPaymentTransition(p.status, to);

  await withPrivilegedDbAccess(`payment.transition:${p.status}->${to}`, async (db) => {
    await db.update(payments).set({ status: to, updatedAt: new Date() }).where(eq(payments.id, p.id));
    await db.insert(paymentEvents).values({
      id: newId('pme'), orgId: p.orgId, paymentId: p.id, orderId: p.orderId, quoteId: p.quoteId,
      eventType: meta?.eventType ?? NAME_FOR[to], fromStatus: p.status, toStatus: to,
      provider: 'stripe', providerEventId: meta?.providerEventId ?? null, payloadSummary: meta?.payloadSummary ?? null,
    });
  });

  await writeAudit({
    action: NAME_FOR[to], orgId: p.orgId, actorUserId: actor.userId, actorRole: actor.role,
    entityType: 'payment', entityId: p.id,
    before: { status: p.status }, after: { status: to, ...(meta?.providerEventId ? { provider_event_id: meta.providerEventId } : {}) },
  });

  await emitPaymentEvent(NAME_FOR[to], { payment_id: p.id, order_id: p.orderId, quote_id: p.quoteId });
  ...
  if (paymentSucceeds(to)) {
    const order = await withPrivilegedDbAccess('payment.succeeded.read_order', async (db) => {
      const rows = await db.select({ status: orders.status }).from(orders).where(eq(orders.id, p.orderId)).limit(1);
      return rows[0] ?? null;
    });
