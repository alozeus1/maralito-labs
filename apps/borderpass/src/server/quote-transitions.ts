...
const EVENT_FOR: Partial<Record<QuoteStatus, string>> = {
  pending_finance_approval: 'quote.submitted_for_approval',
...
export async function transitionQuote(
  ...
  await withPrivilegedDbAccess(`quote.transition:${q.status}->${to}`, async (db) => {
    ...
    if (to === 'declined') {
      ...
      if (meta?.reason) patch.declineReason = meta.reason;
    }
  ...
  await writeAudit({
    ...
    after: { status: to, ...(meta?.extra ?? {}) },
  });
  const evt = EVENT_FOR[to];
  if (evt) await emitQuoteEvent(evt, { quote_id: q.id, order_id: q.orderId });
}
