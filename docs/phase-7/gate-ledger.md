...
| 18 | Env/secrets review + secret rotation | `env-secrets-review.md` | ✅ PASS (rotation done + propagated) | Godwill / 2026-07-08 | Dev-only review green (no server secret in `NEXT_PUBLIC_`, `.env*` gitignored, CI secret-scan + Semgrep 0 findings). **Rotation complete:** owner rotated the DB password + JWT signing key and propagated the new DB password to the app (`.env.local` + Vercel Preview) — verified by the Row 11 smoke reaching the DB. Optional later housekeeping: remove old keys from Supabase "Previously used keys" once nothing depends on them. |
| 19 | Deployment readiness sign-off | `deployment-readiness-checklist.md` | ✅ ACTIVATED (owner Option B, conditions met) | Godwill / 2026-07-08 | Owner's conditional **Option B** (2026-07-05) — synthetic private mobile tester round, "effective only after Row 11 passes with evidence and Row 18's rotation closes" — is now **in effect**: Row 11 ✅ + Row 18 rotation done. Scope: **synthetic-only**; no real PII, no live payments (Row 15 deferred), no staging/pilot (separate higher sign-off). |
