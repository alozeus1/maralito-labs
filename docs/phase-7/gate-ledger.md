...
| 15 | Stripe LIVE validation (before real payments) | operator | 🔲 UNRUN | | Out of scope this session (TEST mode only, by instruction). NOTE: distinct from an end-to-end **TEST**-mode proof, which is BLOCKED by the row-13 bug above. |
