...
import {
  hasRole, hasPermission, isStaff,
  requireAuth, requireRole, requirePermission, requireAdminAccess, requireCustomerAccess,
} from './index';

const customer = buildSession({ authUserId: 'u1', orgId: 'org_1', roles: ['customer'], permissions: ['profile.read'] });
const compliance = buildSession({ authUserId: 'u2', orgId: 'org_1', roles: ['compliance_admin'], permissions: ['order.approve'] });
const superA = buildSession({ authUserId: 'u3', orgId: 'org_1', roles: ['super_admin'], permissions: [] });
...
describe('rbac helpers', () => {
  ...
  it('requireAdminAccess: customer blocked (not_found), staff allowed', () => {
    try { requireAdminAccess(customer); throw new Error('should throw'); }
    catch (e) { expect((e as AuthError).code).toBe('not_found'); }
  ...
  it('requireCustomerAccess: customer ok, compliance forbidden, super_admin ok', () => {
    ...
    try { requireCustomerAccess(compliance); throw new Error('should throw'); }
    catch (e) { expect((e as AuthError).code).toBe('forbidden'); }
