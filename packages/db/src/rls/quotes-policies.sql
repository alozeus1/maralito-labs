...
drop policy if exists quotes_staff_insert on quotes;
...
drop policy if exists qli_customer_select on quote_line_items;
...
drop policy if exists qli_staff_select on quote_line_items;
...
drop policy if exists qli_staff_write on quote_line_items;
...
drop policy if exists qsh_staff_select on quote_status_history;
create policy qsh_staff_select on quote_status_history for select
  using (quote_id in (select id from quotes where org_id = app_current_org_id() and app_is_staff()));
drop policy if exists qap_staff_select on quote_approvals;
