/**
 * Phase 6 (ADR-0012) — inspections RLS isolation on real Postgres (PGlite), applying the REAL
 * policies.sql + orders/quotes/payments/notifications/inspections policy files. Customer own-read +
 * staff org manage; history staff-only; no customer writes.
 */
...
beforeAll(async () => {
  ...
  await db.exec(`
    ...
    create table inspections(id text primary key, org_id text not null, customer_id text not null, order_id text not null, status text not null default 'scheduled', result text, staff_notes text, customer_summary text, scheduled_for timestamptz, completed_at timestamptz, created_at timestamptz default now(), updated_at timestamptz default now());
  ...
  await db.exec(`
    ...
    insert into orders(id,order_ref,customer_id,org_id,status,correlation_id) values('ord_a','BP-1','cust_a','org_a','inspection_pending','ord_a'),('ord_b','BP-2','cust_b','org_a','inspection_pending','ord_b');
    insert into inspections(id,org_id,customer_id,order_id,status,staff_notes,customer_summary) values('insp_a','org_a','cust_a','ord_a','in_progress','internal only','Inspection in progress');
    insert into inspection_status_history(id,org_id,inspection_id,order_id,to_status) values('ish_a','org_a','insp_a','ord_a','in_progress');
  `);
});
