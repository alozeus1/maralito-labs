...
const cfg = readFileSync(
  join(dirname(fileURLToPath(import.meta.url)), '../src/stripe/config.ts'),
  'utf8',
);
const pinned = (cfg.match(/DEFAULT_STRIPE_API_VERSION\s*=\s*'([^']+)'/) || [])[1] ?? 'unknown';

let fail = 0;
const line = (ok, name, note = '') => {
  if (!ok) fail++;
...
line(
  !!sk && !!wh,
  '1 STRIPE_SECRET_KEY + STRIPE_WEBHOOK_SECRET present',
  !sk || !wh ? 'set TEST-mode env to run offline checks' : '',
);
if (sk) {
  if (sk.startsWith('sk_live_'))
    line(
      false,
      '2 secret key is TEST mode (sk_test_)',
      'LIVE key detected — REFUSED; use sk_test_ only',
    );
  else
    line(
      sk.startsWith('sk_test_'),
      '2 secret key is TEST mode (sk_test_)',
      sk.startsWith('sk_test_') ? '' : 'not an sk_test_ key',
    );
}

const configuredVer = process.env.STRIPE_API_VERSION || pinned;
line(
  configuredVer === pinned,
  `3 API version matches pinned default (${pinned})`,
  configuredVer === pinned ? '' : `configured=${configuredVer}; VERIFY against Stripe dashboard`,
);
...
if (wh && sk && !sk.startsWith('sk_live_')) {
  try {
    ...
    let threw = false;
    try {
      stripe.webhooks.constructEvent(JSON.stringify({ id: 'tampered' }), header, wh);
    } catch {
      threw = true;
    }
    line(threw, '5 tampered payload rejected (fail closed)');
  } catch (e) {
    line(false, '4 webhook signature verify (offline)', String(e));
  }
} else {
