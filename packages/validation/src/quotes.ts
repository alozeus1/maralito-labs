...
export const QuoteLineKind = z.enum(['product_cost', 'service_fee', 'delivery_fee', 'estimated_import_tax', 'inspection_fee', 'discount', 'adjustment', 'other']);
...
export const QuoteLineItemInput = z.object({
  ...
  message: 'negative amounts only allowed for discount/adjustment', path: ['total_amount_minor'],
});
...
export const ApproveQuote = z.object({ quote_id: QuoteId, reason: z.string().max(1000).optional() });
...
export const SendQuote = z.object({ quote_id: QuoteId, expires_in_hours: z.number().int().min(1).max(720).optional() });
...
export const QuoteListFilters = z.object({ status: z.string().optional(), cursor: z.string().optional(), limit: z.coerce.number().int().min(1).max(100).default(20) });
